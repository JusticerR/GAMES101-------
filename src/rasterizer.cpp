...
{
    auto v = t.toVector4(); // 这里的 v 存储的是屏幕空间坐标（Viewport Space）

    /* 1. 确定包围盒 (Bounding Box) 
       为什么要这么做？
       如果不限制范围，我们需要检查整个屏幕的所有像素。
       通过计算三角形顶点的最小/最大 x, y，我们只需要检查一个刚好包裹住三角形的矩形区域。
    */
    ...
    // 边界对齐，确保覆盖整个像素
    int min_x_idx = (int)std::floor(min_x);
    int max_x_idx = (int)std::ceil(max_x);
    int min_y_idx = (int)std::floor(min_y);
    int max_y_idx = (int)std::ceil(max_y);

    /* 2. 遍历包围盒内的每一个像素 */
    for (int x = min_x_idx; x < max_x_idx; ++x) {
        for (int y = min_y_idx; y < max_y_idx; ++y) {
            
            /* 3. 像素内检测 (Inside-Triangle Test)
               使用 (x+0.5, y+0.5) 是因为我们采样的是像素中心点。
               insideTriangle 会利用叉乘（Cross Product）来判断点是否在三条边的同一侧。
            */
            if (insideTriangle((float)x + 0.5f, (float)y + 0.5f, t.v)) {
                
                /* 4. 计算重心坐标 (Barycentric Coordinates)
                   alpha, beta, gamma 是这个像素点相对于三个顶点的权重。
                   它们不仅能用来判断点是否在三角形内（都>=0），还是之后所有属性插值的“配方”。
                */
                auto [alpha, beta, gamma] = computeBarycentric2D((float)x + 0.5f, (float)y + 0.5f, t.v);

                /* 5. 透视修正插值 (Perspective Correct Interpolation)
                   这是图形学中的一个重点：由于透视投影是非线性的，我们在 2D 屏幕上看到的“中点”，
                   在 3D 空间中往往不是中点。
                   我们需要利用顶点原本的 depth 值 (v[i].w()) 进行加权校正，才能得到真实属性。
                */
                // 计算该像素在观察空间中真实的深度 Z
                float Z = 1.0f / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                // 计算用于 Z-Buffer 深度测试的插值深度 zp
                float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                zp *= Z;

                /* 6. 深度测试 (Depth Buffer / Z-Buffer)
                   只绘制离相机最近的物体。如果当前像素深度 zp 小于之前存的值，则通过测试。
                */
                int index = get_index(x, y);
                if (zp < depth_buf[index]) {
                    
                    /* 7. 插值顶点属性 
                       我们利用修正后的重心坐标，算出该像素点对应的颜色、法线、纹理坐标和空间位置。
                    */
                    ...
                    if (is_flat_shading) {
                        interpolated_normal = (t.normal[0] + t.normal[1] + t.normal[2]).normalized();
                        interpolated_shadingcoords = (view_pos[0] + view_pos[1] + view_pos[2]) / 3.0f;
                    }

                    /* 8. 着色 (Shading)
                       现在我们有齐了所有的像素数据，打包成 payload，交给 fragment_shader。
                       着色器会根据光照模型（写在 main.cpp 里）算出最终颜色。
                    */
                    ...
                    /* 9. 写入缓冲区 */
                    depth_buf[index] = zp; // 更新深度
                    set_pixel(Eigen::Vector2i(x, y), pixel_color); // 绘制像素
                }
            }
        }
    }
